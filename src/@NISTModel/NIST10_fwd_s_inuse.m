function cor = NIST10_fwd_s( raw, par )
%NIST10_fwd_syThetaK: it assumes the horizontal angle is calculated from
%y-axis
%NIST2_fwd    Scanner forward model polar-to-polar NIST simplified
%
%   C = NIST10_fwd_s(R, par) converts the raw encoder/LiDAR unit readings
...
if ~isstruct(par) || ~isfield(par,'x1z')    % for speed we only check 1 field
    error 'Second input must be a structure as created using NIST10_create';
end
...
hh(2,:) = mod(pi/2- hh(2,:)+2*pi, pi);

face2_raw = (abs(raw(3,:)) >pi/2);    
                                      
                                     
iface2 = find(face2_raw);                                             
                                      
HVm = hh;                             % compose Hm, Vm taking into account
if ~isempty(iface2)                   %   that they are to be equivalent face 1 values
    HVm(1,iface2) = ...
       mod(HVm(1,iface2)+pi, 2*pi);  % mod( H+pi,2pi) and then mapped to (-pi,pi] /     mod(HVm(1,iface2), 2*pi)-pi
    HVm(2,iface2) = ...
       pi-HVm(2,iface2);  % mad( 2pi - V, 2pi) and then mapped to (-pi,pi]
end                                      
...
if ~isempty(iface2)     % bring face 2 points back to face 2 values
    hh(1,iface2) =  mod(hh(1,iface2)+pi, 2*pi); %mod( hh(1,iface2),2*pi )-pi;
    hh(2,iface2) =  pi-hh(2,iface2)- pi;  %mod( pi-hh(2,iface2), 2*pi) - pi;
end
