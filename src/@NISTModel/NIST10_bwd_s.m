function raw = NIST10_bwd_s( cor, par )
%NIST2_bwd    Scanner backward model polar-to-polar NIST simplified
%
%   R = NIST10_bwd_s(C, par) converts the polar coordinates C with respect 
%   to the scanner's own coordinate system (SOCS) into raw encoder/LiDAR 
%   unit readings R using the NIST8 scanner model par. The function is
%   the inverse of NIST3_fwd_s such that 
%                   raw = NIST8_bwd(NIST8_fwd(raw,par),par)
%   and
%                   cor = NIST8_fwd(NIST8_bwd(cor,par),par)
...
%   NIST3_create.
%
%   See also NIST8_create, NIST8_fwd
...
for iit=1:maxit
    ...
    dHmu = ( par.x1z ./ (Rmu .* tanVmu) ...
                + par.x3  ./ (Rmu .* sinVmu) ...
                + (par.x5z - par.x7) ./ tanVmu ...
                + 2*par.x6 ./ sinVmu...
                + par.x1n./Rmu) ;

    dVmu = ( (par.x1n + par.x2) * cosVmu ./ Rmu ...
                + par.x4 ...
                + par.x5n * cosVmu...
                - par.x1z * sinVmu./ Rmu...
                - par.x5z * sinVmu) ;
