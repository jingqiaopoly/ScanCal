classdef pointClouduser < handle
...
% ------------------------------------------------------------------------------
    ...
    methods
        ...
        function obj = pointClouduser(pcData, varargin)
        ...
        p = inputParser;
        p.addRequired( 'pcData'     , @(x) ischar(x) || ismatrix(x));           % same validation fcn as in pointcloudread
        p.addParameter('Label'      , 'noLabel', @(x) ischar(x) || isempty(x));
        p.addParameter('RedPoi'     , [0 0 0]  , @(x) numel(x)==3);
        p.addParameter('Attributes' , []);                                      % validation fcn in pointcloudread
        p.addParameter('HeaderLines', 0);                                       % validation fcn in pointcloudread
        p.addParameter('Filter'     , '');                                      % validation fcn in pointcloudread
        ...
        % try
            [XNonRed, A] = pointcloudread(p.pcData, 'Attributes' , p.Attributes, ...
